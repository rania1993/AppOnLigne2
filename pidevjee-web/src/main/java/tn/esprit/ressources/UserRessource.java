...
public Response deleteUserByid(@PathParam("id") int id){  
	if(usermetier.DeleteUserId(id)){
		return Response.status(Status.OK).entity("profil deleted").build();
	}
		return Response.status(Status.NOT_ACCEPTABLE).entity("profil NOT deleted").build();
